...
function checkadcookieexpire(){
	 ...
	 if (navigator.cookieEnabled){
		 if (document.cookie.indexOf('ohentaiinterad_pass') > -1 ) {
			  return false; 
			}
		 else{
			 setCookie('ohentaiinterad_pass',1,4);
			 return true; 
			 }
		 }
	else{ return false; }	
...
function deleteCookie(){
	...
	document.cookie = "ohentaiinterad_pass; expires=Thu, 18 Dec 2013 12:00:00 UTC; path=/";	
...
function menuabso(){
	...
	$('.sidebaradlinkcontainer').css({'width':200});
	$('.sidebaradlink').css({'width':200-30});
	...
	$('.sidebaradlinkcontainer').css({'width':$(window).width()-20});
	$('.sidebaradlink').css({'width':$(window).width()-20-30});
	
	$('.hometagcontainer .viewalltagbutton').css({'width':($(window).width()-30)});
	
	$('.footerlongdis').css({'width':($(window).width()*0.5-40)});
	...
	$('.hometagcontainer .viewalltagbutton').css({'width':($(window).width()-30)});
	$('.footerlongdis').css({'width':($(window).width()-20)});
